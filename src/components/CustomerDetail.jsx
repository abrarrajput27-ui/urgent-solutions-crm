...
import { ArrowLeft, User, Phone, Mail, Calendar, TrendingUp, Car, Loader2, Star, Eye } from 'lucide-react';
...
const CustomerDetail = () => {
  ...
  const fetchCustomerData = async () => {
    try {
      ...
      // Fetch Customer Bookings History using mobile number
      const { data: bookData, error: bookError } = await supabase
        .from('bookings')
        .select('*')
        .eq('customer_mobile', custData.mobile)
      ...
      <div className="dashboard-layout">
        ...
        <div className="main-content">
          ...
          <div style={{ padding: '2rem' }}>
            <button onClick={() => navigate('/customers')} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontWeight: '600', marginBottom: '1rem' }}>
      ...
      <div className="main-content">
        ...
        <div className="dashboard-content" style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <button onClick={() => navigate('/customers')} className="btn-modal-secondary" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '2rem', marginBottom: '2rem', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '2rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
              <div style={{ background: '#EFF6FF', color: '#3B82F6', width: '80px', height: '80px', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '2rem', fontWeight: '800' }}>
                {customer.name.charAt(0).toUpperCase()}
              </div>
              <div>
                <h1 style={{ margin: '0 0 0.5rem 0', color: '#0F172A', fontSize: '1.8rem', fontWeight: '800' }}>{customer.name}</h1>
                <div style={{ display: 'flex', gap: '1.5rem', color: '#64748B', fontSize: '0.9rem' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><Phone size={16} /> {customer.mobile}</span>
                  {customer.email && <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><Mail size={16} /> {customer.email}</span>}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '2rem' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '1.8rem', fontWeight: '800', color: '#0F172A' }}>{customer.total_bookings}</div>
              </div>
              <div style={{ width: '1px', background: '#E2E8F0' }}></div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#64748B', fontSize: '0.8rem', fontWeight: '600', textTransform: 'uppercase', marginBottom: '0.3rem' }}>Total Revenue</div>
                <div style={{ fontSize: '1.8rem', fontWeight: '800', color: '#16A34A' }}>₹{Number(customer.total_revenue || 0).toLocaleString()}</div>
              </div>
              <div style={{ width: '1px', background: '#E2E8F0' }}></div>
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '1.8rem', fontWeight: '800', color: '#F59E0B', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                  {Number(customer.rating || 5).toFixed(1)} <Star size={20} fill="#F59E0B" />
                </div>
              </div>
            </div>
          </div>
          ...
          <h3 style={{ color: '#0F172A', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Calendar size={20} color="#3B82F6" /> Booking History
          </h3>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.9rem' }}>
                  <thead style={{ background: '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}>
                    <tr>
                      ...
                      <th style={{ padding: '1rem', color: '#475569', fontWeight: '600' }}>Vehicle</th>
                  ...
                  <tbody>
                    {bookings.map((b) => (
                      <tr key={b.id} style={{ borderBottom: '1px solid #F1F5F9', transition: 'background 0.2s', ':hover': { background: '#F8FAFC' } }}>
                        ...
                        <td style={{ padding: '1rem', fontWeight: '600', color: '#16A34A' }}>₹{b.my_amount?.toLocaleString() || 0}</td>
