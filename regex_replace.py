...
# Replace the whole map block
pattern = re.compile(r'\{sortedBookings\.map\(\(b\) => \(\s*<div key=\{b\.id\} className="booking-card".*?\)(?=\s*</div>\s*\)\s*\}</div>)', re.DOTALL)

new_card = '''{sortedBookings.map((b) => (
                    <div key={b.id} className="booking-card" style={{ background: "white", borderRadius: "12px", border: "1px solid #E2E8F0", overflow: "hidden", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", display: "flex", flexDirection: "column", width: "100%", boxSizing: "border-box" }}>
                      ...
                      <div style={{ padding: '1rem', flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem', marginTop: 'auto' }}>
                          <button onClick={() => navigate(/bookings/)} style={{ background: '#EFF6FF', color: '#2563EB', border: 'none', padding: '0.5rem', borderRadius: '6px', fontWeight: '700', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.2rem' }}><Eye size={14}/> View</button>
                          {activeTab === 'All' ? (
                            <>
                              <button onClick={() => navigate(/bookings//edit)} style={{ background: '#FFFBEB', color: '#D97706', border: 'none', padding: '0.5rem', borderRadius: '6px', fontWeight: '700', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.2rem' }}><Edit3 size={14}/> Edit</button>
                              <button onClick={() => handleDelete(b.id)} style={{ background: '#FEF2F2', color: '#DC2626', border: 'none', padding: '0.5rem', borderRadius: '6px', fontWeight: '700', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.2rem' }}><Trash2 size={14}/> Delete</button>
                            </>
                          ) : (
                            <>
...
new_content = pattern.sub(new_card, content)
